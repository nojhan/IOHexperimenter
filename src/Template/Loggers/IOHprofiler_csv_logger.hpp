...
template<class T>
void IOHprofiler_csv_logger<T>::set_parameters_name(const std::vector<std::string > &parameters_name) {
  if (this->logging_parameters.size() != 0) {
    this->logging_parameters.clear();
  }
...
///Only for python wrapper.
template<class T>
void IOHprofiler_csv_logger<T>::set_parameters_name(const std::vector<std::string > &parameters_name, const std::vector<double> &initial_parameters) {
...
///Only for python wrapper.
template<class T>
void IOHprofiler_csv_logger<T>::set_parameters(const std::vector<std::string > &parameters_name, const std::vector<double> &parameters) {
  if (parameters_name.size() != parameters.size()) {
    IOH_error("Parameters and their names are given with different size.");
  }
  for (size_t i = 0; i != parameters_name.size(); ++i)
...
///Only for python wrapper.
template<class T>
void IOHprofiler_csv_logger<T>::set_dynamic_attributes_name(const std::vector<std::string > &attributes_name) {
  if (this->attr_per_run_name_value.size() != 0) {
    this->attr_per_run_name_value.clear();
  }
  //default value set as -9999.
...
///Only for python wrapper.
template<class T>
void IOHprofiler_csv_logger<T>::set_dynamic_attributes_name(const std::vector<std::string > &attributes_name, const std::vector<double> &initial_attributes) {
...
///Only for python wrapper.
template<class T>
void IOHprofiler_csv_logger<T>::set_dynamic_attributes(const std::vector<std::string > &attributes_name, const std::vector<double> &attributes) {
  if (attributes_name.size() != attributes.size()) {
    IOH_error("Attributes and their names are given with different size.");
  }
  for (size_t i = 0; i != attributes_name.size(); ++i)
